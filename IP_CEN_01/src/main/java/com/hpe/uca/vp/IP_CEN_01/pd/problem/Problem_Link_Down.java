 ...
 *
 */
public class Problem_Link_Down extends ProblemDefault implements ProblemInterface {
	
	
	public Problem_Link_Down() {
		super();
		setLog(LoggerFactory.getLogger(Problem_Link_Down.class));
	}
	
	private static final org.slf4j.Logger LOG = LoggerFactory.getLogger(Problem_Link_Down.class);
	@Override
	public boolean isAllCriteriaForProblemAlarmCreation(Group group) throws Exception {
		...
		LOG.debug(" Returned value from isAllCriteriaForProblemAlarmCreation : "+ret);
		if(ret) {
			group.getVar().put("Problem_Class",getClass().getSimpleName());
			//Calculate group variables for RCA Enrichment
			...
			if(queryName!=null && !queryName.isEmpty()){
				...
				//params.put("nodeUniqueId", group.getName().split("<e>")[1].split("</e>")[0].trim());
				LOG.debug("isAllCriteriaForProblemAlarmCreation : nodeInfoQuery: "+nodeInfoQuery + " Params : "+params.toString());
				...
				if (result != null && !result.isEmpty() && result.size()>=1){
					LOG.debug("isAllCriteriaForProblemAlarmCreation : nodeInfoQuery output "+result.toString());
					
					//To get Node Type details 
					String linkType = result.get(0).get("link.LinkRelation");
					String[] linkSep = linkType.split("_");
					String AnodeType = linkSep[0];
					String ZnodeType = linkSep[1];
					...
					group.getVar().put("Link_Rate",(result.get(0).get("link.rate")));
					String Link_Node_Name=result.get(0).get("link.node_name");
					String[] nodeSep = Link_Node_Name.split("\\|");
					String A_Node = nodeSep[0];
					String Z_Node = nodeSep[1];
					group.getVar().put("A_Node",A_Node);
					group.getVar().put("Z_Node",Z_Node);
					String Link_Node_Type=result.get(0).get("link.node_type");
					String[] typeSep = Link_Node_Type.split("\\|");
					String A_Node_Type = typeSep[0];
					String Z_Node_Type = typeSep[1];
					group.getVar().put("A_Node_Type",A_Node_Type);
					group.getVar().put("Z_Node_Type",Z_Node_Type);
					group.getVar().put("Service_Label",(result.get(0).get("link.Service_Label")));
					group.getVar().put("Link_Alias",(result.get(0).get("link.link_alias_name")));
					group.getVar().put("Cust_Name",(result.get(0).get("link.Customer_Name")));
					group.getVar().put("Band_Width",(result.get(0).get("link.Bandwidth")));
					group.getVar().put("Link_Mo",(result.get(0).get("link.externalId")));
					...
					group.getVar().put("Link_Rate","DataNa");
					
					group.getVar().put("A_Node","DataNa");
					group.getVar().put("Z_Node","DataNa");
					group.getVar().put("A_Node_Type","DataNa");
					group.getVar().put("Z_Node_Type","DataNa");
					group.getVar().put("Service_Label","DataNa");
					group.getVar().put("Link_Alias","DataNa");
					group.getVar().put("Cust_Name","DataNa");
					group.getVar().put("Band_Width","DataNa");
					group.getVar().put("Link_Mo","DataNa");
					
					
					group.getVar().put("nodeEnrich","NO Result");
					
					LOG.debug("isAllCriteriaForProblemAlarmCreation :  Result not found "+result.toString()+" ret :"+ret);
					LOG.debug("isAllCriteriaForProblemAlarmCreation: PR will not be created - call to update usertext :"+Trigger.getIdentifier()+"ret : "+ret);
				...
				ret=false;
				LOG.debug("isAllCriteriaForProblemAlarmCreation : no cypher query found "+ queryName+"ret : "+ret);
				group.getVar().put("nodeEnrich","NO QUERY");
				group.getVar().put("Link_Type_A","DataNa");
				group.getVar().put("Link_Type_B","DataNa");
				group.getVar().put("Pop_Type","DataNa");
				group.getVar().put("Link_Rate","DataNa");
			...
			LOG.debug("isAllCriteriaForProblemAlarmCreation: PR will be created for :"+Trigger.getIdentifier()+"ret : "+ret);
		}
	...
	public String calculateCustomAdditionalText(Group group) throws Exception {
		...
		if(problemAlarm == null) {
			String mapper = Common_Util.getPassingFilterParamValue(Trigger, "ProblemAlarm.AdditionalText.CustomMapper", getClass().getSimpleName());
			...
			if(!(group.getVar().containsKey("nodeEnrich"))) { 
				...
				if(queryName!=null && !queryName.isEmpty()){
					...
					String reteriveNodeName = MapperUtils.doMapping(Trigger, group, "reteriveNodeName");
					LOG.debug("isAllCriteriaForProblemAlarmCreation : reteriveNodeName: "+reteriveNodeName);
					
					/*String managedObject = MapperUtils.doMapping(Trigger, group, "calculateNodeManagedObjectFromNode");
					LOG.debug("isAllCriteriaForProblemAlarmCreation : managedObject: "+managedObject);
					
					String[] splits = managedObject.split("\\|");
					LOG.debug("isAllCriteriaForProblemAlarmCreation : Splits : "+splits.length);
					
					LOG.debug("isAllCriteriaForProblemAlarmCreation : Splits[0] : "+splits[0]+"s Splits[1] : "+splits[1]);
					String nodeId = splits[0].split("\"")[1]+"|"+splits[1];*/
					...
					if (result != null && !result.isEmpty() && result.size()>=1){
						LOG.debug("calculateCustomAdditionalText : nodeInfoQuery output "+result.toString());
						...
						LOG.debug("calculateCustomAdditionalText :  Result not found "+result.toString());
						LOG.debug("calculateCustomAdditionalText: PR will not be created - call to update usertext :"+Trigger.getIdentifier()+"ret : ");
			 ...
			 */
			String Anode = (String) group.getVar().get("Link_Type_A");
			String Znode = (String) group.getVar().get("Link_Type_B");
			...
			additionalText = additionalText+"  [ "+Anode+" - "+Znode+" ]"+ adtextBlockSeperator + nodeInfo + adtextBlockSeperator;
	...
	public void whatToDoWhenSubAlarmIsAttachedToGroup(Alarm alarm, Group group) throws Exception {
		...
		if(problemAlarm!=null && alarm!=problemAlarm) {
			...
			//LOG.trace("whatToDoWhenSubAlarmIsAttachedToGroup : recalculated text :"+additonalText);
	...
	/*@Override
	public void whatToDoWhenSubAlarmIsCleared(Alarm alarm, Group group) throws Exception {
		// TODO Auto-generated method stub
		super.whatToDoWhenSubAlarmIsCleared(alarm, group);
		LOG.debug("whatToDoWhenSubAlarmIsCleared : "+alarm.getIdentifier());
		Alarm problemAlarm = group.getProblemAlarm();
		if(problemAlarm!=null) {
			String additonalText = calculateCustomAdditionalText(group);
			//LOG.trace("whatToDoWhenSubAlarmIsCleared : recalculated text :"+additonalText);
			Common_Util.updateAlarmAdditionalText(getScenario(), problemAlarm, additonalText);
		}
	...
	@Override
	public void whatToDoWhenSubAlarmIsNoMoreEligible(Alarm alarm, Group group) throws Exception {
		// TODO Auto-generated method stub
	...
	public boolean calculateIfProblemAlarmhasToBeCleared(Group group) throws Exception {
		// TODO Auto-generated method stub
		return super.calculateIfProblemAlarmhasToBeCleared(group);
	}
	...
	public void whatToDoWhenProblemAlarmIsNoMoreEligible(Group group) throws Exception {
        ...
        if ((null != PA) && (ProblemState.HANDLED != PA.getProblemState())) {
        	...
        	String strCreationTime = PA.getCustomFieldValue("creationTimestamp");
            String strClearanceTime = PA.getCustomFieldValue("clearanceTimeStamp");
            String strTerminationTime = PA.getCustomFieldValue("terminationTimeStamp");
            LOG.debug("Alarms in Group Count :"+group.getNbAlarmsSinceCreation());
            LOG.debug("whatToDoWhenProblemAlarmIsNoMoreEligible : creationTimestamp = "+strCreationTime+" , clearanceTimeStamp = "+strClearanceTime+" , terminationTimeStamp = "+strTerminationTime);
            
            if(strCreationTime!=null && !strCreationTime.isEmpty() && strClearanceTime!=null && !strClearanceTime.isEmpty() &&   strTerminationTime!=null && !strTerminationTime.isEmpty()) 
            {
	            Date CreationTime=new Date();  
	            Date ClearanceTime=new Date();
	            Date TerminationTime=new Date();
	            
	          //2019-04-02T07:47:08.730Z
	            SimpleDateFormat temipDF = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSX"); 
	            
	            
	            CreationTime = temipDF.parse(strCreationTime);
	            ClearanceTime = temipDF.parse(strClearanceTime);
	            TerminationTime = temipDF.parse(strTerminationTime);
	            //long diffinMills = (ClearanceTime.getTime() - CreationTime.getTime());
	            long diffinMills = (TerminationTime.getTime() - CreationTime.getTime());
	            LOG.debug("After Changing the format  : creationTimestamp = "+CreationTime+" , clearanceTimeStamp = "+ClearanceTime+" , terminationTimeStamp = "+TerminationTime);
	            LOG.debug("whatToDoWhenProblemAlarmIsNoMoreEligible : Difference between creation and clearance time : "+ diffinMills);
	            
	    		Alarm Trigger = group.getTriggerAlarm();
	   		
	    		long mapperTime = 0L;
	    		String strfluctuationWindow = Common_Util.getPassingFilterParamValue(Trigger, "ProblemAlarm.FluctuationWindow", getClass().getSimpleName());
	    		 LOG.debug("whatToDoWhenProblemAlarmIsNoMoreEligible : ProblemAlarm.FluctuationWindow : "+ strfluctuationWindow);
	    		if(strfluctuationWindow!=null && !strfluctuationWindow.isEmpty()){
	    			 if(strfluctuationWindow.contains("m")) {
	    					strfluctuationWindow = strfluctuationWindow.replaceAll("m", "");
	    					 LOG.debug("strfluctuationWindow if m :"+strfluctuationWindow);
	    					mapperTime = Integer.parseInt(strfluctuationWindow)*60*1000;
	    			 }else if(strfluctuationWindow.contains("s")) {
	    					strfluctuationWindow = strfluctuationWindow.replaceAll("s", "");
	    					System.out.println(strfluctuationWindow);
	    					mapperTime = Integer.parseInt(strfluctuationWindow)*1000;
	    			}
	    		}
	    		
	    		LOG.debug("whatToDoWhenProblemAlarmIsNoMoreEligible : Fluctuation window : "+ mapperTime);
	    		
	    		if(diffinMills<=mapperTime) {
	    			LOG.debug("whatToDoWhenProblemAlarmIsNoMoreEligible : Fluctuation detected! RCA text will be updated for "+ PA.getIdentifier());
		        	String at = PA.getAdditionalText();
		        	at = at.replaceFirst("RCA\\s*:", "RCA FLUCTUATION :");
		        	PA.setAdditionalText(at);
		        	
		            Action a = new Action("TeMIP_AOAction");
		            
		            a.addCommand("directiveName", "SET"); 
		            a.addCommand("entityName", PA.getIdentifier());
		            a.addCommand("additionalText", PA.getAdditionalText());
		
		       		getScenario().addAction(a);
		             
		       		a.executeAsync("Fluctuation");
	    		}
	        }	
        }
	...
	public void whatToDoWhenProblemAlarmIsHandled(Group group) throws Exception {
		...
		LOG.info("whatToDoWhenProblemAlarmIsHandled : Alarm ID: "+group.getProblemAlarm().getIdentifier() +
				" Ticket ID : "+group.getTroubleTicketIdentifier()+
				" Ticket Delay : "+group.getTroubleTicketComputedDelay()+
				" Ticket Creation Time : "+group.getTroubleTicketCreationRequestTime());
	...
	public Long computeDelayForTroubleTicketCreation(Event event, Group group) {
		...
		LOG.info("computeDelayForTroubleTicketCreation : Alarm ID : "+alarm.getIdentifier()+" Alarm Additional Text : "+alarm.getAdditionalText()+
		    		" Group : "+group.getProblemAlarm()+" TT Delay : "+problemAlarmDelay);
	...
	public boolean isAllCriteriaForTroubleTicketCreation(Group group) throws Exception {
		...
		if(res){
			LOG.info("isAllCriteriaForTroubleTicketCreation in : Java Class "
					+ group.getProblemAlarm().getIdentifier());
		}
